...
Считаем для (probability, odds) расширенный набор показателей:
  ...
  • ev_pct = (p * odds - 1) * 100 (EV в процентах);
  • kelly = (b*p - q) / b (доля Келли);
  • verdict ∈ {"брать", "осторожно", "не брать"};
  • composite — единый score для сортировки прогнозов по «EV при
...
Цели:
  1. Уйти от сортировки топа по чистой вероятности — пользователь хочет
     «не самый высокий процент, а самый EV, при этом вероятная
     ставка».
...
# проиграть >55%.
MIN_PROB_TAKE = 0.55
# Минимальная EV (EV %), которую считаем «настоящей EV».
...
# EV или вероятность чуть ниже основного порога.
...
# 1.45 — это не EV +795%, это мусор. Защита: если real_odds
...
# Если кф < 1.51 — слишком «очевидная» ставка с минимальным запасом,
# глобально фильтруем по требованию пользователя.
MIN_FAIR_ODDS = 1.51
...
def _composite_score(prob: float, odds: float | None) -> float:
    ...
    Идея: «+EV И вероятный». Берём EV (в долях) и взвешиваем его
    sqrt(p), чтобы при равной EV предпочесть более вероятный
    ...
    ev = prob * odds - 1.0
    if ev <= 0.0:
        # Отрицательную EV не «спасаем» вероятностью — пусть
